...
contract SCHStake is Initializable, UUPSUpgradeable, AccessControl {
    ...
    function _stake_Lock() public _whenPause onlyRole(ADMIN_ROLE) {
        require(stakeLock, "stake lock is already open");
        stakeLock = false;
        emit StakeLock(stakeLock);
    }

    function _stake_Unlock() public _whenPause onlyRole(ADMIN_ROLE) {
    ...
    function _reward_Lock() public _whenPause onlyRole(ADMIN_ROLE) {
        require(rewardLock, "reward lock is already open");
        rewardLock = false;
        emit RewardLock(stakeLock);
    }

    function _reward_Unlock() public _whenPause onlyRole(ADMIN_ROLE) {
